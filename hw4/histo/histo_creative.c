...
long* histogram(char* fn_input) {
  ...
  long results_of_each_thread[256][threads_count];
  double t_start, t_end;

  // results_of_each_thread = (long**)malloc(256 * sizeof(long*));
  ...
  // for (k = 0; k < 256; k++) {
  //   results_of_each_thread[k] = (long*)malloc(1 * sizeof(long));
  //   *results_of_each_thread[k] = 0;
  // }
...
#pragma omp parallel for 
      for (j = 0; j < image->col; j++) {
        results_of_each_thread[image->content[i][j]][omp_get_thread_num()]++;
      }
    }
  }
